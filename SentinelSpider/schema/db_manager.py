...
class DatabaseManager:
    ...
    def show_tables(self):
        ...
        # 分类显示表
        sentinel_spider_tables = []
        mediacrawler_tables = []
        
        for table_name in tables:
            if table_name in ['daily_news', 'daily_topics', 'topic_news_relation', 'crawling_tasks']:
                sentinel_spider_tables.append(table_name)
            else:
                mediacrawler_tables.append(table_name)
        
        data_list_message += "SentinelSpider核心表:"
        data_list_message += "\n"
        for table in sentinel_spider_tables:
...
def main():
    parser = argparse.ArgumentParser(description="SentinelSpider数据库管理工具")
