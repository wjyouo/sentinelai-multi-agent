...
class SentinelSpider:
    ...
    def __init__(self):
        ...
        logger.info("SentinelSpider AI爬虫项目")
    ...
    def initialize_database(self) -> bool:
        ...
        try:
            ...
            # 传递当前环境变量，确保子进程使用正确的配置
            env = os.environ.copy()
            # 强制设置数据库名称，避免子进程读取错误的.env
    ...
    def run_complete_workflow(self, target_date: date = None, platforms: list = None,
        ...
        """运行完整工作流程"""
        logger.info("开始完整的SentinelSpider工作流程")
    ...
    def show_status(self):
        """显示项目状态"""
        logger.info("SentinelSpider项目状态:")
        logger.info(f"项目路径: {self.project_root}")
    ...
    def setup_project(self) -> bool:
        """项目初始化设置"""
        logger.info("开始SentinelSpider项目初始化...")
        ...
        logger.info("SentinelSpider项目初始化完成！")
        return True

def main():
    """命令行入口"""
    parser = argparse.ArgumentParser(description="SentinelSpider - AI爬虫项目主程序")
    ...
    try:
        ...
        if args.setup:
            if spider.setup_project():
                logger.info("项目设置完成，可以开始使用SentinelSpider！")
            else:
        ...
        if args.broad_topic:
            ...
            # 默认运行完整工作流程
            logger.info("运行完整SentinelSpider工作流程...")
            spider.run_complete_workflow(
                target_date, args.platforms, args.keywords_count,
                args.max_keywords, args.max_notes, args.test
            )
