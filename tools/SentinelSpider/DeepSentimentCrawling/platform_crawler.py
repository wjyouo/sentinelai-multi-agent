...
class PlatformCrawler:
    ...
    def configure_mediacrawler_db(self):
        ...
        try:
            ...
            # PostgreSQL配置值：如果使用PostgreSQL则使用SentinelSpider配置，否则使用默认值或环境变量
            pg_password = config.settings.DB_PASSWORD if is_postgresql else "sentiment"
            pg_user = config.settings.DB_USER if is_postgresql else "sentiment"
            ...
            pg_db_name = config.settings.DB_NAME if is_postgresql else "sentiment_db"
            
            # 替换数据库配置 - 使用SentinelSpider的数据库配置
...
# postgresql config - 使用SentinelSpider的数据库配置（如果DB_DIALECT是postgresql）或环境变量
...
'''
            ...
            logger.info(f"已配置MediaCrawler使用SentinelSpider {db_type}数据库")
            return True
