"""
SentinelSpider 数据库初始化（SQLAlchemy 2.x 异步引擎）

此脚本创建 SentinelSpider 扩展表（与 MediaCrawler 原始表分离）。
支持 MySQL 与 PostgreSQL，需已有可连接的数据库实例。

数据模型定义位置：
- SentinelSpider/schema/models_sa.py
"""
...
def _build_database_url() -> str:
    ...
    db_name = settings.DB_NAME or "sentinel_spider"
